...
foreach ($quadro->tasks_ativas as $ta) {
    ...
    $tasks[] = ['id'=>$ta->id , 'task' => $ta->task->task,'dono'=>$ta->user->nome,'prioridade' =>$cor,'pause' => $t,'tempo' => $tempo. ' mins','revisao'=>$ta->task->descricao,'tipo' =>0];
  }
  foreach ($quadro->tasks_em_andamento as $ta) {
    if($ta->prioridade == 0){
      $cor = 'card pan dragg qitem bg-primary blink';
  ...
  foreach ($quadro->tasks_em_revisao as $ta) {
    ...
    $tasks[] = ['id'=>$ta->id , 'task' => $ta->task->task,'dono'=>$ta->user->nome,'prioridade' =>$cor,'revisao'=>$ta->task->descricao,'tempo'=>$hoje,'tipo' =>2];
  }
  foreach ($quadro->tasks_finalizadas as $ta) {
    if($ta->prioridade == 0){
      $cor = 'card pan dragg qitem bg-light';
    ...
    $tasks[] = ['id'=>$ta->id , 'task' => $ta->task->task,'dono'=>$ta->user->nome,'prioridade' =>$cor,'tempo'=>'Resolvido em '.$tempo,'tipo' =>3];
  }
 }
